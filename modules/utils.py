...
def process_train_record(record, tokenizer, max_len, show_oub = False, show_bast = False, show_no_answ = False ):
    ...
    # - tokenizer: the specific tokenizer it needs to be utilized
    # Returns:
    # - [id, input_ids, attention_mask, token_type_ids, start_token_idx, end_token_idx]: if the computation is successfull
    # - ["","","","","", ""]: if the computation went wrong
    ...
    if len(ans_token_idx) == 0:
        if show_no_answ:
            ...
            print("Question: \t{} \n".format(question))
            print("Paragraph:\t{}\n".format(context))
            print("Given Answer:\t{}".format(answer))
            print("==============================================================\n\n\n\n")
    ...
    if padding_length > 0:  # pad
        ...
        if show_bast:
            print("tokenized_question:\t" + str(len(tokenized_question.input_ids)))
            print("tokenized context:\t" + str(len(tokenized_context.input_ids)))
            print("input ids: \t\t"+ str(len(input_ids)))
            print("==============================================================\n\n")
...
def print_prediction(model, tokenizer, test_set):
    df = process_dataset(test_set, tokenizer, answer_available=True, max_len=512)
    x, _ = dataframe_to_array(df)
    p = model.predict(x)
    '''
    TODO: questa cosa ÃƒÂ¨ da cani ma se uso iterrows gli indici non iniziano da uno e questo ÃƒÂ¨ il modo per mettere in pari p e row
    '''
    first_idx = test_set.first_valid_index()
    for i, row in enumerate(zip(p[0],p[1])):

        start = row[0]
        end = row[1]
        id = test_set["id"][i+first_idx]
        context = test_set["context"][i+first_idx]
        question = test_set["question"][i+first_idx]
        answer = test_set["answer_text"][i+first_idx]
        print("id:\t\t{} \n".format(id))
        print("Question: \t{} \n".format(question))
        print("Paragraph:\t{}\n".format(context))
        print("True Answer:\t{}".format(answer))
        print("Possib Answer:\t{}".format(*tokenizer.convert_ids_to_tokens(x_val[0][i-first_idx][start: end+1])))
        print("==============================================================\n\n")
